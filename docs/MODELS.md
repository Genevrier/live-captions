...
- Tencent's official [Hy-MT2 7B GGUF](https://huggingface.co/tencent/Hy-MT2-7B-GGUF) revision is `ab8472660ac61fac25f1af43fac2599d52a8a775` (Apache-2.0). Q6_K is the Honor MAX QUALITY default and Q4_K_M is an explicit alternative. Exact URLs, sizes and SHA-256 values are in `translation-models.json`. Tencent does not publish a Q5 file in this revision, so no Q5 choice is advertised. These models use the same pinned llama.cpp CPU path as 1.8B; OpenCL is not enabled. [Q4 host smoke](validation/hymt7b-q4-host-smoke.txt) and [Q6 host smoke](validation/hymt7b-q6-host-smoke.txt) passed the shared C++ bridge; phone latency/accuracy benchmarks remain open.
